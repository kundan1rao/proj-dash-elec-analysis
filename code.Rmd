...
packages <- c("plotly","readxl","flexdashboard","dplyr","ggplot2","sf","tmap","readr","DT","ggridges","ggthemes","knitr")
if(!require(packages)){
...
DT::datatable(win_data_temp,options = list(pageLength=20, scrollY=F
))
```
...
DT::datatable(AC_cand,options = list(pageLength=20, scrollY=F
))
```
...
```{r}
#Number Of Candidates Fielded Party wise
DT::datatable(as.data.frame(table(dataset3$PARTY)),options = list( scrollY=F
))
```
...
```{r}
#Winning Candidates party wise
DT::datatable(as.data.frame(table(win_data_temp$PARTY)),options = list(scrollY=F
))
```
...
DT::datatable(as.data.frame(table(win_data_temp_w$PARTY)),options = list(scrollY=F
))
```
...
DT::datatable(as.data.frame(table(win_data_temp_m$PARTY)),options = list( scrollY=F
))
...
```{r}
#Winning candidates Gender Wise
knitr::kable(as.data.frame(table(win_data_temp$SEX)))
```
...
```{r}
#Winning Candidates Catagory wise
knitr::kable(as.data.frame(table(win_data_temp$CATEGORY)))
```
